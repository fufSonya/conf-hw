...
def _format_value(v: Value) -> str:
    if isinstance(v, int):
        return str(v)
    if isinstance(v, dict):
        # Only supported in TOML as inline tables, but our writer uses tables instead.
...
def _assert_toml_array_is_valid(items: list[Value]) -> None:
    # TOML arrays must be homogeneous. We enforce it for correctness.
    if not items:
        return
    ...
    if first_tag == "dict":
        # These should be written as array-of-tables, not inline arrays.
